...
export class StoreController {
    static async getPublicProducts(req: Request, res: Response) {
        try {
            ...
            const countQueryText = `SELECT COUNT(*) FROM products p LEFT JOIN stores s ON p.store_id = s.id WHERE` + queryText.split('WHERE')[1];
    ...
    static async getMyStores(req: any, res: Response) {
        try {
            ...
            const result = await query(`SELECT * FROM stores WHERE owner_id = $1 ORDER BY created_at DESC`, [userId]);
    ...
    static async createStore(req: any, res: Response) {
        try {
            ...
            const finalOwnerId = (roles.includes('ADMIN') && owner_id) ? owner_id : userId;
    ...
    static async updateStore(req: any, res: Response) {
        try {
            ...
            if (storeCheck.rows[0].owner_id !== userId && !userRoles.includes('ADMIN')) {
