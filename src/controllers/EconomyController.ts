...
export class EconomyController {
    ...
    static async triggerSemesterMinting(req: any, res: Response) {
        try {
             ...
             const configResult = await query("SELECT * FROM economy_config WHERE key LIKE 'ROLE_MINT_%'");
             ...
             for (const user of usersResult.rows) {
                 ...
                 for (const role of roles) {
                     if (roleLimits[role]) maxAmount = Math.max(maxAmount, roleLimits[role]);
                 }
                 if (maxAmount === 0 && roleLimits['ESTUDIANTE']) maxAmount = roleLimits['ESTUDIANTE'];

                 if (maxAmount > 0) {
                     distributionPlan.push({ walletId: user.wallet_id, amount: maxAmount });
                     totalNeeded += maxAmount;
                 }
             }

             console.log(`Economy Plan: Need ${totalNeeded} UC for ${distributionPlan.length} users.`);
             ...
             */
             ...
             res.json({ 
                 message: 'Semester minting completed. Funds stored in Treasury Reserve.', 
