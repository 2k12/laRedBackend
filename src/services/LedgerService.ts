...
export class LedgerService {
  ...
  async getBalance(walletId: string): Promise<number> {
      const res = await query('SELECT COUNT(*) FROM coins WHERE wallet_id = $1 AND status = $2', [walletId, 'ACTIVE']);
  ...
  async mintTokens(toWalletId: string, amount: number, referenceId?: string): Promise<Transaction> {
    const lastTxRes = await query('SELECT hash FROM transactions ORDER BY created_at DESC LIMIT 1');
  ...
  async transferTokens(fromWalletId: string, toWalletId: string, amount: number, referenceId?: string): Promise<Transaction> {
    const client = await (await import('../config/db')).default.connect();
    ...
    try {
      ...
      if (coinsRes.rows.length < amount) {
          throw new Error(`Insufficient funds. Available: ${coinsRes.rows.length}, Required: ${amount}`);
      }
      ...
      const txRes = await client.query(
        ...
        (from_wallet_id, to_wallet_id, amount, type, reference_id, previous_hash, hash) 
        VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
        [fromWalletId, toWalletId, amount, 'TRANSFER', referenceId, prevHash, hash]
      );
      const txId = txRes.rows[0].id;
